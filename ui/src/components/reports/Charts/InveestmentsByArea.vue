<template>
  <v-card>
    <v-card-title>Investment Distribution by Flagship Program</v-card-title>
    <v-card-subtitle>Total investment allocated per Flagship Program</v-card-subtitle>
...
<script setup>
  ...
  const fetchData = async () => {
    loading.value = true;
    try {
      let url = `/project/getInvestmentsByArea`
  ...
  const updateChart = () => {
    ...
    const series = (() => {
      if (chartType.value === 'pie') {
        return [{
          name: 'Investment by Flagship Program',
          type: 'pie',
          radius: ['40%', '70%'],
          label: { formatter: '{b}: {d}%' },
          data,
